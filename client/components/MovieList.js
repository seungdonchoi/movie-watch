  ...
  render() {
    ...
    return (
        <li className={theMovie.watched ? "watched" : ""}>
          <h2>{theMovie.title}  {theMovie.imdbLink && <a taget="_blank">IMDB</a>}</h2>
          <ul className="genres-list">
            {theMovie.genres.map((genre) => {
              return <li key={genre.id}><a>{genre.name}</a></li>
            })}
          </ul>
          {/* ${theMovie.watched === false ? `<a class="watch-link" href="/movies/${movie.id}/mark-watched">I watched this!</a>` : ""} */}
        </li>
    )
  }
}
